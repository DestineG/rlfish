...
def mcts_search(game, iters):
    ...
    raw_visits = {f"{child.move[0]},{child.move[1]}": child.visits for child in root.children}
    raw_values = {f"{child.move[0]},{child.move[1]}": round(child.value / child.visits, 4) for child in root.children}
...
class DataCollector:
    def __init__(self, game, iters=50000, data_log_path="train_data/gomoku_game.jsonl"):
        self.game = game
        self.iters = iters
    ...
    def record_step(self, board, player, mcts_visits, mcts_values):
        step_snapshot = {
            ...
            "player": player,
            "mcts_raw_visits": mcts_visits,
            "mcts_raw_values": mcts_values
        }
        self.current_game_moves.append(step_snapshot)

    def save_full_game(self, winner):
        game_data = {
            "board_size": self.game.map_size[0],
            "winner": winner,
    ...
    def run(self):
        ...
        while not game_over:
            ...
            if self.game.apply_move(move):
                ...
                self.save_full_game(winner=0) # 平局
                game_over = True
...
if __name__ == "__main__":
    ...
    for ep in range(episodes):
        ...
        collector = DataCollector(my_game, iters=MCTS_ITERS, data_log_path=filepath)
        collector.run()