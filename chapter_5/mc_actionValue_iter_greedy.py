...
def greedy_probs(Q, state, action_size):
    ...
    action_probs = {a: 0.0 for a in range(action_size)}
    action_probs[max_q] = 1.0
...
class Agent:
    ...
    def update(self):
        ...
        for state, action, reward in reversed(self.memory):
            ...
            alpha = 1 / self.cnts[(state, action)]
...
def mc_eval():
    ...
    agent = Agent()
    num_episodes = 5000
    max_steps = 100  # 每个回合最多走100步 避免因为策略 动态变化&贪婪 导致无限循环
    ...
    for i in range(env.height):
        for j in range(env.width):
            state = (i, j)
            V[i, j] = max([agent.Q[(state, a)] for a in env.actions()])

    env.render_v(V, title="Final State Value Function")

    env.render_q(agent.pi, title="Final Policy (Action Directions)")
